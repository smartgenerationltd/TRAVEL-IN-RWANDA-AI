...
const App: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (prompt: string) => {
    ...
    try {
      ...
      const match = fullResponse.match(/\[MAP:(-?\d+\.?\d*),(-?\d+\.?\d*),(.*?)\]/);
      if (match) {
        const [, lat, lng, name] = match;
        setDestination({ lat: parseFloat(lat), lng: parseFloat(lng), name: name.trim() });
      }
    ...
    >
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        <aside className="md:w-2/5 xl:w-1/3 h-64 md:h-auto border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700">
           <MapComponent destination={destination} onGetDirections={handleGetDirections} />
        </aside>
